...
The `doc/` directory contains the project website built with MkDocs. The three conceptual pages
(`doc/docs/streams.md`, `doc/docs/reading-and-writing.md`, `doc/docs/bit-operations.md`) are
checked in to source control. The API reference pages under `doc/docs/API/` are **generated by a
tool and are not checked in** — do not create or edit them manually.
